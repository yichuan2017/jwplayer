    ...
    describe('API Config', function() {
        ...
        describe('preload', function() {
            ...
            it('should be none in item when set in top level', function() {
                expect(new Config({ preload: 'none' }).playlist[0].preload)
                    .to.equal('none');
            });
            ...
            it('should be auto in item when set in top level', function() {
                expect(new Config({ preload: 'auto' }).playlist[0].preload)
                    .to.equal('auto');
            });

            it('should default to meta for invalid values in first item', function() {
                expect(new Config({
                    preload: 'invalid',
                    playlist: [{}]
            ...
            it('should override config in first item over top level', function() {
                ...
                }).playlist[0].preload).to.equal('none');
            });
            ...
            it('should be none when set in first playlist item', function() {
                expect(new Config({ playlist: [{ preload: 'none' }] }).playlist[0].preload)
                    .to.equal('none');
            });
            ...
            it('should be auto when set in first playlist item', function() {
                ...
                }).playlist[0].preload).to.equal('auto');
            });

            it('should ignore when set in second playlist item', function() {
                expect(new Config({
                    playlist: [{
                        preload: 'none'
                    }, {
